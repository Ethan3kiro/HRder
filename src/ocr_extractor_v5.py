"""
OCR提取模块 V5 - 基于 EasyOCR 的智能提取

使用深度学习 OCR 引擎（EasyOCR）提升识别率
"""
import re
...
logger = logging.getLogger("transmitter.ocr_v5")


class OCRExtractorV5:
    """OCR数据提取器 V5 - 基于 EasyOCR 的智能提取"""
    ...
    def extract_from_image(self, image_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            combiner_data = self._find_combiner_numbers(all_numbers)
            items.extend(combiner_data)
            
            # 提取 Z-Plane 数据
            for module in self.ZPLANE_MODULES:
                logger.info(f"匹配 Z-Plane {module} 数据...")
                zplane_data = self._find_zplane_numbers(all_numbers, module)
    ...
    def _find_combiner_numbers(self, numbers: List[Dict]) -> List[Dict]:
        ...
        策略：COMBINER 数据通常在图像上半部分（y < 0.4），横向排列
        """
    ...
    def _find_zplane_numbers(self, numbers: List[Dict], module: str) -> List[Dict]:
        ...
        策略：Z-Plane 数据在图像下半部分（y > 0.4），分为4列（A, B, C, D）
        """
