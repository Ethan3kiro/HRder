...
logger = logging.getLogger("transmitter.ocr_v6")


class OCRExtractorV6:
    """OCR数据提取器 V6 - 优化版 EasyOCR 智能提取"""
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("正在初始化 EasyOCR V6...")
            self.reader = easyocr.Reader(['en'], gpu=False, verbose=False)
            logger.info("✓ EasyOCR V6 初始化成功")
    ...
    def extract_from_image(self, image_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            combiner_data = self._find_combiner_numbers_v6(all_numbers)
            items.extend(combiner_data)
            
            # 提取 Z-Plane 数据
            for module in self.ZPLANE_MODULES:
                logger.info(f"匹配 Z-Plane {module} 数据...")
                zplane_data = self._find_zplane_numbers_v6(all_numbers, module)
    ...
    def _find_combiner_numbers_v6(self, numbers: List[Dict]) -> List[Dict]:
        """
        V6 改进版：使用 DBSCAN 聚类找出 COMBINER 数据
        
        改进：
        1. 使用 DBSCAN 聚类而不是简单的容差匹配
        2. 更智能的行识别
        3. 支持更多的 COMBINER 项
        """
        # COMBINER 区域大致在 y < 0.4 的位置
        combiner_candidates = [n for n in numbers if n['y_center'] < 0.4]
        
        if not combiner_candidates:
            logger.warning("未找到 COMBINER 候选数字")
            return []
        
        # 使用 DBSCAN 聚类按 Y 坐标分组
        if len(combiner_candidates) < 2:
            rows = [combiner_candidates]
        else:
            y_coords = np.array([[n['y_center']] for n in combiner_candidates])
            clustering = DBSCAN(eps=0.04, min_samples=1).fit(y_coords)
            
            # 按聚类标签分组
            rows = []
            for label in set(clustering.labels_):
                row = [combiner_candidates[i] for i, l in enumerate(clustering.labels_) if l == label]
                rows.append(row)
    ...
    def _find_zplane_numbers_v6(self, numbers: List[Dict], module: str) -> List[Dict]:
        """
        V6 改进版：使用 DBSCAN 聚类和动态 X 范围找出 Z-Plane 数据
        
        改进：
        1. 使用 DBSCAN 聚类按 Y 坐标分组
        2. 动态调整 X 坐标范围（增加重叠）
        3. 更智能的 Current/ISO Temp 列识别
        """
        # Z-Plane 区域大致在 y > 0.4 的位置
        zplane_candidates = [n for n in numbers if n['y_center'] > 0.4]
        ...
        x_start = max(0, module_index * 0.25 - 0.05)  # 向左扩展 5%
        x_end = min(1.0, (module_index + 1) * 0.25 + 0.05)  # 向右扩展 5%
        ...
        logger.info(f"找到 Z-Plane {module} 的 {len(module_numbers)} 个候选数字")
        
        # 使用 DBSCAN 聚类按 Y 坐标分组
        if len(module_numbers) < 2:
            rows = [[num] for num in module_numbers]
        else:
            y_coords = np.array([[n['y_center']] for n in module_numbers])
            # 使用更大的 eps 以捕获更多行
            clustering = DBSCAN(eps=0.04, min_samples=1).fit(y_coords)
            ...
            rows = []
            for label in set(clustering.labels_):
                row = [module_numbers[i] for i, l in enumerate(clustering.labels_) if l == label]
                rows.append(row)
        ...
        for row_num, row in enumerate(rows, 1):
            ...
            # 智能识别 Current 和 ISO Temp 列
            # 策略：Current 列通常在左侧，ISO Temp 列在右侧
            if len(row) >= 1:
                # 第一个数字是 Current
                items.append({
            ...
            # 如果有更多数字，记录但不使用（可能是识别错误）
            if len(row) > 2:
                logger.debug(f"Z-Plane {module} Row {row_num} 有额外的 {len(row)-2} 个数字，已忽略")
