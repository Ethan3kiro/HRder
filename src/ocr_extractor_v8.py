...
主要改进：
1. 基于数值特征过滤校准因子（极小值 <0.01 和重复的 15.15）
2. 基于位置过滤底部区域（y > 0.85）
3. 改进 Z-Plane 行分配逻辑
4. 优化 Z-Plane D 识别
"""
...
logger = logging.getLogger("transmitter.ocr_v8")


class OCRExtractorV8:
    """OCR数据提取器 V8 - 改进行分配逻辑"""
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("正在初始化 EasyOCR V8...")
            self.reader = easyocr.Reader(['en'], gpu=False, verbose=False)
            logger.info("✓ EasyOCR V8 初始化成功")
    ...
    def _find_combiner_numbers_v8(self, numbers: List[Dict]) -> List[Dict]:
        """
        V8 版本：找出 COMBINER 数据
        
        改进：
        1. 使用过滤后的数字（已排除底部校准因子）
        2. 保持 V7 的容差设置
        """
    ...
    def _find_zplane_numbers_v8(self, numbers: List[Dict], module: str) -> List[Dict]:
        """
        V8 改进版：智能过滤校准因子，改进 Z-Plane 数据识别
        
        改进：
        1. 过滤校准因子（极小值 <0.01 和特定值如 15.15, 332, 412）
        2. 过滤底部区域（y > 0.85）
        3. 对 Z-Plane D 使用更宽松的 X 范围
        """
