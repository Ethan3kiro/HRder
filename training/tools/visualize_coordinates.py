...
"""
坐标可视化工具
帮助调整 prepare_dl_data.py 中的坐标定义
"""
...
def visualize_coordinates(image_path, label_path, output_path):
    """在图像上可视化坐标框"""
    # 加载图像
...
def main():
    ...
    # 生成可视化
    output_path = Path(f"coordinate_visualization_{image_path.stem}.png")
    success = visualize_coordinates(image_path, label_file, output_path)
    
    if success:
        print("\n" + "=" * 60)
        print("✅ 完成！")
        print("=" * 60)
        print(f"\n请查看生成的图像: {output_path}")
        print("\n绿色框显示了当前定义的坐标位置")
        print("如果框的位置不正确，请调整 tools/prepare_dl_data.py 中的坐标")
        print("\n调整步骤:")
        print("1. 打开生成的图像，查看绿色框是否对准数字")
        print("2. 如果不对准，编辑 tools/prepare_dl_data.py")
        print("3. 找到 get_cell_coordinates() 方法")
        print("4. 调整坐标值（base_x, base_y, spacing 等）")
        print("5. 重新运行此脚本查看效果")
        print("6. 重复直到所有框都对准")
        print("7. 然后重新运行训练: ./train_dl_ocr.sh")
